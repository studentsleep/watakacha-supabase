        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 relative">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div x-show="isGuest" x-transition class="mt-4 p-4 rounded-xl border flex items-start gap-4 shadow-sm bg-white border-gray-300">
                            ...
                            <div class="flex-grow">
                                ...
                                <ul class="mt-2 space-y-1 text-sm text-gray-600">
                                    <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 bg-gray-400 rounded-full"></span><span>ไม่ได้เป็นสมาชิกของทางร้าน</span></li>
                                </ul>
                            </div>
                            <button @click="resetMember" class="text-gray-400 hover:text-red-500 transition p-2 hover:bg-gray-100 rounded-full">✕</button>
                        </div>
                    ...
                    {{-- 2. เลือกสินค้า และ อุปกรณ์เสริม --}}
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    ...
                                    <template x-for="(item, index) in cart" :key="'item-'+item.id">
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                {{-- [แก้] ใช้ SVG ตรงๆ แทน data-lucide เพื่อแก้ปัญหาปุ่มหาย --}}
                                    ...
                                    <tr x-show="cart.length === 0 && accessoryCart.length === 0">
                                        <td colspan="5" class="px-4 py-10 text-center text-gray-400">
                                            <span class="font-medium">ยังไม่มีสินค้าในตะกร้า</span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="bg-white p-4 rounded-lg border border-gray-200 space-y-3 shadow-sm">
                                <div>
                                    ...
                                    <input type="date" x-model="rentalDate" @change="updateReturnDate" class="block w-full mt-1 rounded-md border-gray-300 text-sm">
                                </div>
                                <div>
                                    <span class="text-xs text-gray-500 font-bold uppercase tracking-wider">กำหนดคืน</span>
                                    <input type="date" x-model="returnDate" class="block w-full mt-1 rounded-md border-gray-300 text-sm">
                                </div>
                            </div>
                            ...
                            <div class="text-sm space-y-3 text-gray-600 bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                                <div class="flex justify-between items-center py-1">
                                    <span>ค่าชุดหลัก <span class="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full border border-gray-200" x-text="cartItemCount"></span></span>
                                    <span class="font-bold text-gray-800" x-text="formatPrice(cartTotal)"></span>
                                </div>
                                
                                <div class="flex justify-between items-center py-1" x-show="accessoryCart.length > 0">
                                    <span>อุปกรณ์เสริม <span class="text-xs bg-orange-100 text-orange-500 px-2 py-0.5 rounded-full border border-orange-200" x-text="accessoryItemCount"></span></span>
                                    <span class="font-bold text-gray-800" x-text="formatPrice(accessoryTotal)"></span>
                                </div>
                                ...
                                <template x-if="makeupPrice > 0">
                                    <div class="flex justify-between items-center bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                                        <div class="flex flex-col"><span class="font-semibold text-gray-700">ค่าแต่งหน้า</span><span class="text-xs text-gray-500" x-text="getMakeupName()"></span></div>
                                        <span class="font-bold text-gray-700" x-text="formatPrice(makeupPrice)"></span>
                                    </div>
                                </template>
                                <template x-if="packagePrice > 0">
                                    <div class="flex justify-between items-center bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                                        <div class="flex flex-col"><span class="font-semibold text-gray-700">ค่าถ่ายภาพ</span><span class="text-xs text-gray-500" x-text="getPackageName()"></span></div>
                                ...
                                <template x-if="discountAmount > 0">
                                    <div class="flex justify-between items-center bg-green-50 p-2.5 rounded-lg border border-green-100">
                                        <div class="flex flex-col"><span class="font-bold text-green-700 flex items-center gap-1">🏷️ ส่วนลด</span><span class="text-xs text-green-600" x-text="getPromotionName()"></span></div>
                            ...
                            <div class="border-t-2 border-dashed border-gray-300 pt-4 mt-2 space-y-3">
                                <div class="flex justify-between items-end">
                                    <span class="text-gray-500 font-medium pb-1">ยอดรวมทั้งสิ้น</span>
                                    <span class="text-xl font-bold text-gray-800" x-text="formatPrice(grandTotal)"></span>
                                </div>
                                <div class="bg-blue-50 p-3 rounded-lg border border-blue-200">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-blue-800 font-bold flex items-center gap-2">มัดจำ (50%)</span>
                                        <span class="text-2xl font-extrabold text-blue-700" x-text="formatPrice(depositAmount)"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="pt-2">
                                <button @click="openConfirmModal" 
                                    class="w-full py-3.5 bg-gray-800 hover:bg-gray-900 text-white font-bold rounded-xl shadow-lg transform transition hover:-translate-y-0.5 flex justify-center items-center gap-2"
                                    :disabled="(!member && !isGuest) || (cart.length === 0 && accessoryCart.length === 0) || isSubmitting">
                                    <span x-text="isSubmitting ? 'กำลังบันทึก...' : 'ชำระมัดจำ & ยืนยันการเช่า'"></span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {{-- 🟢 MODAL 1: ยืนยันการทำรายการ --}}
            <div x-show="showConfirm" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
                <div class="flex items-center justify-center min-h-screen px-4 text-center">
                    <div class="fixed inset-0 transition-opacity bg-gray-900 bg-opacity-50" @click="showConfirm = false"></div>
                    <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative z-50">
                        <h3 class="text-xl font-bold leading-6 text-gray-900 mb-4">ยืนยันการเช่า?</h3>
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4 space-y-2">
                            <div class="flex justify-between text-gray-600">
                                <span>ยอดรวม:</span><span class="font-bold" x-text="formatPrice(grandTotal)"></span>
                            </div>
                            <div class="flex justify-between text-blue-700 font-bold text-lg border-t pt-2 border-gray-200">
                                <span>มัดจำที่ต้องจ่าย:</span><span x-text="formatPrice(depositAmount)"></span>
                            </div>
                        </div>
                        <div class="mt-6 flex justify-end gap-3">
                            <button @click="showConfirm = false" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">ยกเลิก</button>
                            <button @click="processSubmission" class="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-bold shadow">ยืนยันและบันทึก</button>
                        </div>
                    </div>
                </div>
            </div>

            {{-- 🟢 MODAL 2: แจ้งเตือนทั่วไป --}}
            <div x-show="showAlert" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
                <div class="flex items-center justify-center min-h-screen px-4 text-center">
                    ...
                    <div class="inline-block w-full max-w-sm p-6 my-8 overflow-hidden text-center align-middle transition-all transform bg-white shadow-xl rounded-2xl relative z-50">
                        <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full mb-4" :class="alertType === 'success' ? 'bg-green-100' : 'bg-red-100'">
                             <span x-show="alertType === 'success'" class="text-2xl">✅</span>
                             <span x-show="alertType === 'error'" class="text-2xl">❌</span>
                        </div>
                        <h3 class="text-lg font-medium leading-6 text-gray-900" x-text="alertTitle"></h3>
                        <div class="mt-2"><p class="text-sm text-gray-500" x-text="alertMessage"></p></div>
                        <div class="mt-5"><button @click="closeAlert" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-gray-800 border border-transparent rounded-md shadow-sm hover:bg-gray-700 focus:outline-none sm:text-sm">ตกลง</button></div>
                    </div>
                </div>
            </div>

            {{-- 🟢 [ใหม่] MODAL 4: ยืนยันการลบสินค้า (Delete Confirmation) --}}
                ...
                <div class="flex items-center justify-center min-h-screen px-4 text-center">
                    ...
                    <div class="inline-block w-full max-w-sm p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative z-50">
                        <div class="flex items-center gap-3 mb-4 text-red-600">
                            <div class="bg-red-100 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></div>
                            <h3 class="text-lg font-bold">ยืนยันการลบ?</h3>
                        </div>
                        <p class="text-gray-600 mb-6">คุณต้องการลบรายการนี้ออกจากตะกร้าใช่หรือไม่?</p>
                        <div class="flex justify-end gap-3">
                            <button @click="showDeleteModal = false" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">ยกเลิก</button>
                            <button @click="confirmDelete" class="px-4 py-2 text-white bg-red-600 rounded-lg hover:bg-red-700 font-bold">ลบรายการ</button>
                        </div>
                    </div>
                </div>
            </div>

            {{-- 🟢 MODAL 3: ใบเสร็จ (Receipt / Slip) --}}
                    ...
                    <div id="receipt-modal" class="inline-block w-full max-w-sm overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-sm relative z-[70] my-8 font-mono text-sm">
                        ...
                        <div class="p-6 bg-white relative">
                            ...
                            <div class="space-y-2 mb-4 border-b border-dashed border-gray-300 pb-4">
                                ...
                                <div class="flex justify-between"><span class="text-gray-500">ลูกค้า:</span><span class="font-bold text-gray-800" x-text="member ? (member.first_name + ' ' + member.last_name) : 'ลูกค้าทั่วไป'"></span></div>
                        ...
                        <div class="bg-gray-100 p-4 border-t border-gray-200 no-print">
                            <button @click="closeReceipt" class="w-full py-3 bg-gray-900 hover:bg-black text-white font-bold rounded shadow-lg uppercase tracking-wide">พิมพ์ใบเสร็จ / ปิดหน้าต่าง</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        function rentalSystem(initData) {
            return {
                ...
                // [ใหม่] ตัวแปรสำหรับ Delete Modal
                ...
                resetMember() { this.member = null; this.isGuest = false; this.cart = []; this.accessoryCart = []; this.selectedPromotionId = ''; this.selectedMakeupId = ''; this.selectedPackageId = ''; this.selectedPhotographerId = ''; },
                ...
                // [แก้ไข] ใช้ askDeleteItem แทน confirm()
                decreaseQty(index) { 
                ...
                // [ใหม่] ฟังก์ชันจัดการ Delete Modal
                ...
                get grandTotal() {
                    let total = this.cartTotal + this.accessoryTotal - this.discountAmount + this.makeupPrice + this.packagePrice;
                    ...
                    try {
                        const res = await fetch(`{{ route('reception.storeRental') }}`, {
                            ...
                            body: JSON.stringify({
                                ...
                                rental_date: this.rentalDate, return_date: this.returnDate,
