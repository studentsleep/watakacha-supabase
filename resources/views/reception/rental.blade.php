<x-app-layout>
    ...
    {{-- เริ่ม x-data ตรงนี้เลย เพื่อให้ครอบคลุมทั้งหน้า --}}
                                            ...
                                            <li @click="addToCart(item); showItemsDropdown = false" class="px-4 py-3 hover:bg-gray-100 cursor-pointer flex justify-between items-center border-b border-gray-100 last:border-0 transition group">
                                                <div>
                                                    ...
                                                    <div class="flex items-center gap-2 mt-0.5">
                                                        ...
                                                        {{-- แสดงจำนวนที่ว่าง (คำนวณจาก Controller) --}}
                                                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded border"
                                                            ...
                                                            x-text="'ว่าง: ' + item.available_stock + ' (จาก ' + item.stock + ')'">
                                                        </span>
                                                    </div>
                                                </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-200 sticky top-6">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="bg-white p-4 rounded-lg border border-gray-200 space-y-3 shadow-sm">
                                {{-- วันที่เช่า (เลือกได้) --}}
                                <div>
                                    <span class="text-xs text-gray-500 font-bold uppercase tracking-wider">วันที่เช่า (เริ่ม 7 วัน)</span>
                                    <input type="date" x-model="rentalDate" @change="updateReturnDate" class="block w-full mt-1 rounded-md border-gray-300 text-sm focus:border-gray-500 focus:ring-gray-500 font-bold text-blue-700">
                                </div>

                                {{-- กำหนดคืน (Auto - แก้ไม่ได้) --}}
                                <div>
                                    <span class="text-xs text-gray-500 font-bold uppercase tracking-wider">กำหนดคืน (อัตโนมัติ 7 วัน)</span>
                                    <div class="block w-full mt-1 px-3 py-2 bg-gray-100 rounded-md border border-gray-300 text-sm text-gray-600 font-medium">
                                ...
                                {{-- Warning 3 Days Buffer --}}
                                <div class="mt-2">
                                    <span class="text-xs text-orange-500 font-bold uppercase tracking-wider">ระยะเวลาดูแลชุด (ห้ามเช่าต่อ)</span>
                                    <div class="text-xs text-orange-600 mt-1">
                                        หลังจากคืนแล้ว ระบบจะล็อกชุดไว้ 3 วันเพื่อซักและซ่อมแซม
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-sm space-y-3 text-gray-600 bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                                ...
                                <template x-if="makeupPrice > 0">
                                    <div class="flex justify-between items-center bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                                        <div class="flex flex-col"><span class="font-semibold text-gray-700">ค่าแต่งหน้า</span><span class="text-xs text-gray-500" x-text="getMakeupName()"></span></div><span class="font-bold text-gray-700" x-text="formatPrice(makeupPrice)"></span>
                                    </div>
                                </template>
                                <template x-if="packagePrice > 0">
                                    <div class="flex justify-between items-center bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                                        <div class="flex flex-col"><span class="font-semibold text-gray-700">ค่าถ่ายภาพ</span><span class="text-xs text-gray-500" x-text="getPackageName()"></span></div><span class="font-bold text-gray-700" x-text="formatPrice(packagePrice)"></span>
                                    </div>
                                </template>
                                <template x-if="discountAmount > 0">
                                    <div class="flex justify-between items-center bg-green-50 p-2.5 rounded-lg border border-green-100">
                                        <div class="flex flex-col"><span class="font-bold text-green-700 flex items-center gap-1">🏷️ ส่วนลด</span><span class="text-xs text-green-600" x-text="getPromotionName()"></span></div><span class="font-bold text-green-700" x-text="'-' + formatPrice(discountAmount)"></span>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="border-t-2 border-dashed border-gray-300 pt-4 mt-2 space-y-3">
                                ...
                                <div class="bg-blue-50 p-3 rounded-lg border border-blue-200">
                                    <div class="flex justify-between items-center mb-1"><span class="text-blue-800 font-bold flex items-center gap-2">มัดจำ (50%)</span><span class="text-2xl font-extrabold text-blue-700" x-text="formatPrice(depositAmount)"></span></div>
                                </div>
                            </div>
                ...
                <div class="flex items-center justify-center min-h-screen px-4 text-center">
                    ...
                    <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative z-50">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4 space-y-2">
                            <div class="flex justify-between text-gray-600"><span>ยอดรวม:</span><span class="font-bold" x-text="formatPrice(grandTotal)"></span></div>
                            <div class="flex justify-between text-blue-700 font-bold text-lg border-t pt-2 border-gray-200"><span>มัดจำที่ต้องจ่าย:</span><span x-text="formatPrice(depositAmount)"></span></div>
                        </div>
                        <div class="mt-6 flex justify-end gap-3"><button @click="showConfirm = false" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">ยกเลิก</button><button @click="processSubmission" class="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-bold shadow">ยืนยันและบันทึก</button></div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="text-right">
                                        <template x-if="member">
                                            <div><span class="font-bold text-gray-800 block" x-text="member.first_name + ' ' + member.last_name"></span><span class="text-gray-500 text-[10px] block" x-text="'Tel: ' + member.tel"></span></div>
                                        </template>
                                        <template x-if="!member">
                                            <div><span class="font-bold text-gray-800 block">ลูกค้าทั่วไป (Guest)</span><span class="text-gray-600 text-[10px] block" x-text="'คุณ' + guestName + ' โทร ' + guestPhone"></span></div>
                                        </template>
                                    </div>
                                </div>
                                ...
                                <table class="w-full text-xs">
                                    ...
                                    <tbody class="text-gray-700">
                                        <template x-for="item in cart" :key="'r-item-'+item.id">
                                            <tr>
                                                <td class="py-1" x-text="item.item_name"></td>
                                                <td class="text-center py-1" x-text="item.quantity"></td>
                                                <td class="text-right py-1" x-text="formatPrice(item.price * item.quantity)"></td>
                                            </tr>
                                        </template>
                                        <template x-for="acc in accessoryCart" :key="'r-acc-'+acc.id">
                                            <tr>
                                                <td class="py-1" x-text="acc.name + ' (Acc)'"></td>
                                                <td class="text-center py-1" x-text="acc.quantity"></td>
                                                <td class="text-right py-1" x-text="formatPrice(acc.price * acc.quantity)"></td>
                                            </tr>
                                        </template>
                                        <tr x-show="makeupPrice > 0">
                                            <td class="py-1">แต่งหน้า</td>
                                            <td class="text-center py-1">1</td>
                                            <td class="text-right py-1" x-text="formatPrice(makeupPrice)"></td>
                                        </tr>
                                        <tr x-show="packagePrice > 0">
                                            <td class="py-1">ถ่ายภาพ</td>
                                            <td class="text-center py-1">1</td>
                                            <td class="text-right py-1" x-text="formatPrice(packagePrice)"></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        function rentalSystem(initData) {
            return {
                ...
                updateReturnDate() {
                    ...
                    // [ปรับแก้] บวก 6 วัน (รวมวันเริ่ม = 7 วัน)
