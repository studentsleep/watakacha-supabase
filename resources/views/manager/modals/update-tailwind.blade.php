        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full"
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div class="mt-4 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                    ...
                    <!-- Form อัปโหลดรูปใหม่ -->
                    <div>
                        <x-input-label for="new_image-{{ $item->id }}" :value="__('เพิ่มรูปภาพใหม่')" />
                        {{-- [CRUD ใหม่] ส่ง Form ไปยัง Route ใหม่ --}}
                        <form action="{{ route('manager.items.uploadImage', $item->id) }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-2 mt-1">
                            @csrf
                            <input type="file" name="image" id="new_image-{{ $item->id }}" required class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-100 dark:file:bg-gray-700 file:text-gray-700 dark:file:text-gray-300 hover:file:bg-gray-200 dark:hover:file:bg-gray-600">
