                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-4 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="username-{{ $user->user_id }}" :value="__('ชื่อผู้ใช้')" class="after:content-['*'] after:text-red-500 after:ml-0.5"/>
                                <x-text-input type="text" name="username" id="username-{{ $user->user_id }}" value="{{ $user->username }}" required class="mt-1 block w-full" />
                            </div>
                            <div>
                                <x-input-label for="email-{{ $user->user_id }}" :value="__('อีเมล')" class="after:content-['*'] after:text-red-500 after:ml-0.5"/>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="first_name-{{ $user->user_id }}" :value="__('ชื่อจริง')" class="after:content-['*'] after:text-red-500 after:ml-0.5"/>
                                <x-text-input type="text" name="first_name" id="first_name-{{ $user->user_id }}" value="{{ $user->first_name }}" required class="mt-1 block w-full" />
                            </div>
                            <div>
                                <x-input-label for="last_name-{{ $user->user_id }}" :value="__('นามสกุล')" />
                                <x-text-input type="text" name="last_name" id="last_name-{{ $user->user_id }}" value="{{ $user->last_name }}"  class="mt-1 block w-full" />
                            </div>
                        </div>

                         <div>
                            <x-input-label for="tel-{{ $user->user_id }}" :value="__('เบอร์มือถือ')" class="after:content-['*'] after:text-red-500 after:ml-0.5"/>
                            <x-text-input type="text" name="tel" id="tel-{{ $user->user_id }}" value="{{ $user->tel }}" class="mt-1 block w-full" require/>
                        </div>

                        <div>
                            <x-input-label for="password-{{ $user->user_id }}" :value="__('รหัสผ่านใหม่ (ว่างไว้หากไม่เปลี่ยน)')" />
                            <x-text-input type="password" name="password" id="password-{{ $user->user_id }}" class="mt-1 block w-full" />
                        </div>
                        
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="user_type_id-{{ $user->user_id }}" :value="__('ประเภทผู้ใช้')" class="after:content-['*'] after:text-red-500 after:ml-0.5"/>
                            ...
                            <div>
                                <x-input-label for="status-{{ $user->user_id }}" :value="__('สถานะ')" />
                                <select name="status" id="status-{{ $user->user_id }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="active" @selected($user->status == 'active')>กำลังใช้งาน</option>
                                    <option value="inactive" @selected($user->status == 'inactive')>ระงับการใช้งาน</option>
                                </select>
                            </div>
                        </div>
