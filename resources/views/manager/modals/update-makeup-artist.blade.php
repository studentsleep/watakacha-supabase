        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="document" onclick="event.stopPropagation()">
            ...
            <form action="{{ route('manager.makeup_artists.update', $artist->makeup_id) }}" method="POST">
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100 mb-4">แก้ไข: {{ $artist->first_name }}</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="status" id="status-{{ $artist->makeup_id }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300">
                                <option value="active" @selected($artist->status == 'active')>เปิดใช้งาน</option>
                                <option value="inactive" @selected($artist->status == 'inactive')>ระงับการใช้งาน</option>
                            </select>
                        </div>
                ...
                {{-- ปุ่มอยู่ภายใน form และภายใน inline-block --}}
                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <x-primary-button type="submit" class="w-full sm:w-auto sm:ml-3">บันทึกแก้ไข</x-primary-button>
                    <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700" onclick="toggleModal('updateMakeupArtistModal-{{ $artist->makeup_id }}', false)">
                        ยกเลิก
                    </button>
                </div>
