                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-4 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <x-input-label for="last_name-{{ $member->member_id }}" :value="__('Last Name')" />
                                <x-text-input type="text" name="last_name" id="last_name-{{ $member->member_id }}" value="{{ $member->last_name }}" required class="mt-1 block w-full" />
                            </div>
                        </div>
                         <div>
                            <x-input-label for="tel-{{ $member->member_id }}" :value="__('Telephone')" />
                            <x-text-input type="text" name="tel" id="tel-{{ $member->member_id }}" value="{{ $member->tel }}" class="mt-1 block w-full" />
                        </div>
                        
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             {{-- ( ... points, status ... ) --}}
                             <div>
                                <x-input-label for="points-{{ $member->member_id }}" :value="__('Points')" />
                                <x-text-input type="number" name="points" id="points-{{ $member->member_id }}" value="{{ $member->points }}" required class="mt-1 block w-full" />
                            </div>
                            <div>
                                <x-input-label for="status-{{ $member->member_id }}" :value="__('Status')" />
