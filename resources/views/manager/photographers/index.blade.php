    ...
    <div class="py-8 bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-800 shadow-lg rounded-xl overflow-hidden border border-gray-700">
                <table class="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-700 bg-gray-800">
                        ...
                        <tr class="hover:bg-gray-700/50 transition">
                            ...
                            <td class="px-6 py-4 font-bold text-indigo-300 text-base">{{ $pg->first_name }} {{ $pg->last_name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-400">
                                <div>{{ $pg->tel }}</div>
                                <div class="text-xs text-gray-500">{{ $pg->email }}</div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-1 rounded-full text-xs font-bold {{ $pg->status == 'active' ? 'bg-green-900 text-green-300 border border-green-700' : 'bg-red-900 text-red-300 border border-red-700' }}">
                                    {{ $pg->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right font-medium">
                                <button onclick="toggleModal('updatePhotographerModal-{{ $pg->photographer_id }}', true)" class="text-blue-400 hover:text-blue-300 mr-2"><i data-lucide="file-pen-line" class="w-5 h-5"></i></button>
                                <form action="{{ route('manager.photographers.destroy', $pg->photographer_id) }}" method="POST" class="inline-block" onsubmit="return confirm('ลบ?')">
                                    @csrf @method('DELETE')
                ...
                @if($photographers->hasPages()) <div class="px-6 py-4 bg-gray-800 border-t border-gray-700">{{ $photographers->links() }}</div> @endif
            </div>

            @include('manager.modals.add-photographer')
            @foreach($photographers as $pg) @include('manager.modals.update-photographer', ['photographer' => $pg]) @endforeach
        </div>
    </div>
</x-app-layout>