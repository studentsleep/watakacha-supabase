    ...
    <div class="py-8 bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-800 shadow-lg rounded-xl overflow-hidden border border-gray-700">
                <table class="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-700 bg-gray-800">
                        ...
                        <tr class="hover:bg-gray-700/50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-bold text-indigo-300 text-base mb-1" title="{{ $artist->description }}">{{ $artist->first_name }} {{ $artist->last_name }}</div>
                            </td>

                            {{-- ✅ แก้ไขส่วนติดต่อ ให้มีไอคอนและ Line ID สวยๆ --}}
                            <td class="px-6 py-4 text-sm text-gray-400">
                                ...
                                @if($artist->email)
                                <div class="text-xs text-gray-500 ml-5">{{ $artist->email }}</div>
                                @endif
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-full text-xs font-bold {{ $artist->status == 'active' ? 'bg-green-900 text-green-300 border border-green-700' : 'bg-red-900 text-red-300 border border-red-700' }}">
                                    {{ $artist->status == 'active' ? 'เปิดใช้งาน' : 'ระงับการใช้งาน' }}
                                </span>
                            </td>

                            <td class="px-6 py-4 text-right font-medium">
                                <button onclick="toggleModal('updateMakeupArtistModal-{{ $artist->makeup_id }}', true)" class="text-blue-400 hover:text-blue-300 mr-2">
                                    <i data-lucide="file-pen-line" class="w-5 h-5"></i>
                                </button>

                                {{-- Form ลบแบบ SweetAlert --}}
                                <form id="delete-form-{{ $artist->makeup_id }}"
                                    action="{{ route('manager.makeup_artists.destroy', $artist->makeup_id) }}"
                                    method="POST"
                                    class="inline-block">
                                    ...
                                    <button type="button"
                                        onclick="confirmDelete('delete-form-{{ $artist->makeup_id }}')"
                ...
                @if($makeup_artists->hasPages())
                <div class="px-6 py-4 bg-gray-800 border-t border-gray-700">{{ $makeup_artists->links() }}</div>
                @endif
            </div>
